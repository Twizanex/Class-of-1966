...
function TIMELINE_toplevel_page()
{
	...
	if(isset($_POST[submit]))
	{
		...
		$tl_cover =  $_POST[tl_cover];
		$tl_cover_video =  $_POST[tl_cover_video];
		...
		update_option("tl_cover",$tl_cover);
		update_option("tl_cover_video",$tl_cover_video);
	...
	if(get_option('tl_cover')=="") update_option('tl_cover',"img");
	...
	echo "
			...
			<form method='post' name='options'>
				<table class='form-table'>
					...
					<tr>
						...
						if(get_option("tl_cover")=="img"){$video_selected = "";}else{$img_selected= "";}
					echo "
						<select name='tl_cover' id='tl_cover'>
							<option $img_selected  value='img'>image</option>
							<option $video_selected value='video'>video</option>
						</select><br>
						<input class='regular-text' type='text' id='tl_cover_video' name='tl_cover_video' value='".get_option("tl_cover_video")."'/>
						</td>
					</tr>
		...
		<script>
		jQuery(function(){
			jQuery('#tl_cover').change(function(){
				showCoverInfo();
			});
		});
		function showCoverInfo()
		{
			if( jQuery('#tl_cover :selected').val() == 'img' )
			{
				jQuery('#tl_cover_video').hide().val('GPlB9pHk51s');
			}
			else
			{
				jQuery('#tl_cover_video').show();
			}
		}
		showCoverInfo();
		</script>
